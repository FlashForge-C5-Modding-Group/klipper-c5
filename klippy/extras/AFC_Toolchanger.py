...
class AfcToolchanger(afcUnit):
    ...
    def system_Test(self, cur_lane: AFCLane, delay: float, assignTcmd: str, enable_movement: bool):
        ...
        msg = ""
        filament_loaded = (cur_lane.prep_state
                           and cur_lane.load_state
                           and cur_lane.extruder_obj.lane_loaded == cur_lane.name)
        if cur_lane.extruder_obj.creator5_tool_index is not None:
            msg = ("<span class=success--text>FILAMENT PRESENT</span>" if
                   cur_lane.extruder_obj.tool_start_state else
                   "<span class=primary--text>FILAMENT ABSENT</span>")
            msg += cur_lane.extruder_obj.prep_on_shuttle_check(cur_lane)
        elif filament_loaded:
