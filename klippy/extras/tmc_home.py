...
class TmcHome:
    def __init__(self, config):
        ...
        gcode.register_command('TMCHOME_X', self.cmd_TMCHOME_X, desc=self.cmd_TMCHOME_help)
        gcode.register_command('TMCHOME_Y', self.cmd_TMCHOME_Y, desc=self.cmd_TMCHOME_help)
    ...
    def run_probe(self, gcmd, num):
        ...
        return pos
    cmd_TMCHOME_help = "measure XY position mm value"
    def cmd_TMCHOME_X(self, gcmd):
        pos = self.run_probe(gcmd, 0)
        gcmd.respond_info("Result is x=%.6f" % (pos[0],))
        self.last_z_result = pos[2]
    def cmd_TMCHOME_Y(self, gcmd):
        pos = self.run_probe(gcmd, 1)
        gcmd.respond_info("Result is x=%.6f" % (pos[1],))
        self.last_z_result = pos[2]

def load_config(config):
    return TmcHome(config)
